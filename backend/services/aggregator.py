...
def invalidate_summary_cache():
    ...
    _CACHE.clear()
    invalidate_fname_type_map()  # force reclassification after new data


def _inject_db(filters: dict, db: Session) -> dict:
    """Return a shallow copy of filters with db injected for fast facility_type lookup."""
    return {**filters, "_db": db}


# ── Facility type Python classifier (mirrors the Excel IFS formula) ──────────
# PERFORMANCE: We classify facility names in Python (only ~300 unique names),
# then translate facility_type filters into fast `facility_name IN (...)` clauses.
# This avoids running 20+ LIKE conditions on every one of 350k+ rows.

def classify_facility_name(name: str) -> str:
    """Classify one facility name into its type (mirrors Excel IFS order)."""
    if not name:
        return "No Match"
    padded = f" {name} "
    if " 30June Intl Centr-Nephrlgy N Urolgy " in padded:
        return "ICNU"
    nl = padded.lower()
    if " hospital " in nl or " hospi " in nl or " hs " in nl:
        return "Hospital"
    if " complex " in nl:
        return "Complex"
    # PCU keywords (case-insensitive partial names kept original-case in padded)
    pcu_keywords = [
        " Al Mahsama Al Gadida ", " Al Kabahi Al Gharbi ", " Alwadi Alakhdar ",
        " unit ", " Ismailia unit-Aswan ", " pcu ",
        " Naga El Baraka ", " Nagaa El Khotabaa ", " Om Khalaf ",
        " Omar In Elkattab- Suez ",
    ]
    for kw in pcu_keywords:
        if kw.lower() in nl:
            return "PCU"
    # PCC keywords
    pcc_keywords = [
        " Al Ismailia Algadida ", " pcc ", " center ", " Nefesha ",
        " AL shaheed Mahmoud Nasser Mahgoub C ",
        " Bahr El Bakar Al Gadeda ", " Bahr el Bakar- Old ",
    ]
    for kw in pcc_keywords:
        if kw.lower() in nl:
            return "PCC"
    return "No Match"


# Process-level cache for the facility-name→type map.
# Keyed by the DB's current facility_name set (refreshed when cache clears).
_FNAME_TYPE_MAP: dict[str, str] = {}  # {facility_name: type}
_FNAME_TYPE_MAP_LOADED = False


def _ensure_fname_type_map(db: Session) -> dict[str, str]:
    """Return (cached) {facility_name: facility_type} for every unique name in DB."""
    global _FNAME_TYPE_MAP, _FNAME_TYPE_MAP_LOADED
    if _FNAME_TYPE_MAP_LOADED:
        return _FNAME_TYPE_MAP
    rows = db.execute(text(
        "SELECT DISTINCT facility_name FROM practitioner_records "
        "WHERE facility_name IS NOT NULL AND TRIM(facility_name) != ''"
    )).fetchall()
    _FNAME_TYPE_MAP = {r[0]: classify_facility_name(r[0]) for r in rows}
    _FNAME_TYPE_MAP_LOADED = True
    return _FNAME_TYPE_MAP


def invalidate_fname_type_map():
    """Call after data import so the map is rebuilt on next request."""
    global _FNAME_TYPE_MAP, _FNAME_TYPE_MAP_LOADED
    _FNAME_TYPE_MAP = {}
    _FNAME_TYPE_MAP_LOADED = False


def get_facility_names_for_types(db: Session, type_list: list[str]) -> list[str]:
    """Return all facility names whose type is in type_list. Fast Python lookup."""
    fmap = _ensure_fname_type_map(db)
    return [name for name, ftype in fmap.items() if ftype in type_list]


# Keep the SQL expression for use only in get_facility_type_summary (GROUP BY).
_FACILITY_TYPE_SQL = """
    CASE
        WHEN ' ' || {col} || ' ' LIKE '% 30June Intl Centr-Nephrlgy N Urolgy %' THEN 'ICNU'
        WHEN lower(' ' || {col} || ' ') LIKE '% hospital %'
          OR lower(' ' || {col} || ' ') LIKE '% hospi %'
          OR lower(' ' || {col} || ' ') LIKE '% hs %'                             THEN 'Hospital'
        WHEN lower(' ' || {col} || ' ') LIKE '% complex %'                        THEN 'Complex'
        WHEN lower(' ' || {col} || ' ') LIKE '% al mahsama al gadida %'
          OR lower(' ' || {col} || ' ') LIKE '% al kabahi al gharbi %'
          OR lower(' ' || {col} || ' ') LIKE '% alwadi alakhdar %'
          OR lower(' ' || {col} || ' ') LIKE '% unit %'
          OR lower(' ' || {col} || ' ') LIKE '% ismailia unit-aswan %'
          OR lower(' ' || {col} || ' ') LIKE '% pcu %'
          OR lower(' ' || {col} || ' ') LIKE '% naga el baraka %'
          OR lower(' ' || {col} || ' ') LIKE '% nagaa el khotabaa %'
          OR lower(' ' || {col} || ' ') LIKE '% om khalaf %'
          OR lower(' ' || {col} || ' ') LIKE '% omar in elkattab- suez %'         THEN 'PCU'
        WHEN lower(' ' || {col} || ' ') LIKE '% al ismailia algadida %'
          OR lower(' ' || {col} || ' ') LIKE '% pcc %'
          OR lower(' ' || {col} || ' ') LIKE '% center %'
          OR lower(' ' || {col} || ' ') LIKE '% nefesha %'
          OR lower(' ' || {col} || ' ') LIKE '% al shaheed mahmoud nasser mahgoub c %'
          OR lower(' ' || {col} || ' ') LIKE '% bahr el bakar al gadeda %'
          OR lower(' ' || {col} || ' ') LIKE '% bahr el bakar- old %'             THEN 'PCC'
        ELSE 'No Match'
    END
"""


def _facility_type_expr(alias: str = "") -> str:
    col = f"{alias}.facility_name" if alias else "facility_name"
    return _FACILITY_TYPE_SQL.format(col=col).strip()
...
def _build_where(filters: dict, table_alias: str = "") -> tuple[str, dict]:
    ...
    # facility_type filter — translated to facility_name IN (...) for SPEED.
    # The _db argument is smuggled through filters as "_db" when available.
    # If not available we fall back to the slow CASE expression (only for cascade
    # endpoints where db is not passed into _build_where).
    if filters.get("facility_type"):
        ft_list = filters["facility_type"]
        if not isinstance(ft_list, list):
            ft_list = [ft_list]
        _db = filters.get("_db")  # injected by get_kpi_summary / get_pivot
        if _db is not None:
            matching_names = get_facility_names_for_types(_db, ft_list)
            if matching_names:
                p_names = []
                for i, fname in enumerate(matching_names):
                    p_name = f"_ft_fname_{i}"
                    p_names.append(f":{p_name}")
                    params[p_name] = fname
                clauses.append(f"AND {pfx}facility_name IN ({', '.join(p_names)})")
            else:
                # No facility matches — force zero results
                clauses.append("AND 1=0")
        else:
            # Fallback: slow CASE (used for cascade /filters/options endpoint)
            ftype_expr = _facility_type_expr(table_alias)
            p_names = []
            for i, ftype in enumerate(ft_list):
                p_name = f"facility_type_{i}"
                p_names.append(f":{p_name}")
                params[p_name] = ftype
            clauses.append(f"AND ({ftype_expr}) IN ({', '.join(p_names)})")
...
def get_kpi_summary(db: Session, filters: dict) -> dict:
    ...
    """
    cache_key = f"kpi:{json.dumps({k:v for k,v in filters.items() if k!='_db'}, sort_keys=True)}"
    cached = _cache_get(cache_key)
    if cached is not None:
        return cached

    f = _inject_db(filters, db)
    where, params = _build_where(f)
    e_expr, i_expr, o_expr = _build_metric_exprs(f)
    total_expr = f"({e_expr} + {i_expr} + {o_expr})"

    # Context filters (no practitioner / search) for denominator
    filters_ctx = _inject_db({k: v for k, v in filters.items() if k not in ["practitioner_id", "search"]}, db)
...
def get_pivot(
    ...
    """
    cache_key = f"pivot:{json.dumps({k:v for k,v in filters.items() if k!='_db'}, sort_keys=True)}:{group_by}:{include_top_facs}"
    cached = _cache_get(cache_key)
    if cached is not None:
        return cached
    ...
    allowed = {
        ...
        "facility_type",  # virtual pivot dimension
    }
    if group_by not in allowed:
        group_by = "practitioner_name"

    # For facility_type as group_by dimension, we compute it via CASE expression
    if group_by == "facility_type":
        ftype_sql = _facility_type_expr("pr")
        group_by_clause  = f"({ftype_sql})"
        dimension_expr   = ftype_sql
        cte_dim_expr     = _facility_type_expr()
        cte_group_clause = f"({_facility_type_expr()}), facility_name"
        join_dim         = f"({ftype_sql})"
    elif group_by == "practitioner_name":
    ...
    f_with_db = _inject_db(filters, db)
    where_main, params = _build_where(f_with_db, table_alias="pr")
    ...
    where_fac, params_fac = _build_where_facility_only(_inject_db(filters, db))
    ...
    where_cte, params_cte = _build_where(_inject_db(filters, db))
...
def get_facility_breakdown_table(
    ...
    cache_key = f"breakdown:{json.dumps({k:v for k,v in filters.items() if k!='_db'}, sort_keys=True)}:{group_by}:{row_cap_val}"
    cached = _cache_get(cache_key)
    if cached is not None:
        return cached
    ...
    f_with_db = _inject_db(filters, db)
    where, params = _build_where(f_with_db, table_alias="pr")

    where_fac, params_fac = _build_where_facility_only(f_with_db)
    prefixed_params_fac: dict = {}
    prefixed_where_fac = where_fac
    for k, v in params_fac.items():
        nk = f"brk_{k}"
...
def get_distinct_values_filtered(db: Session, column: str, filters: dict) -> list[str]:
    allowed = {
        "region", "facility_name", "practitioner_id",
        "practitioner_name", "speciality", "visit_date", "month",
        "facility_type",  # virtual computed column
    }
    if column not in allowed:
        return []

    cache_key = f"distinct:{column}:{json.dumps({k:v for k,v in filters.items() if k!='_db'}, sort_keys=True)}"
    cached = _cache_get(cache_key)
    if cached is not None:
        return cached

    if column == "facility_type":
        # facility_type is a computed virtual column — return the fixed set directly
        # from the Python classifier without an extra DB round-trip.
        result = sorted(["PCU", "PCC", "Hospital", "Complex", "ICNU"])
    else:
        where, params = _build_where(_inject_db(filters, db))
        sql = text(f"""
...
def get_facility_type_summary(db: Session, filters: dict) -> list[dict]:
    """Return aggregated counts grouped by computed facility_type.

    This is a single lightweight GROUP BY query — it does NOT do the heavy
    top-4-facility CTE, so it completes in < 1 s even on 350k+ rows.
    Results are ordered by total_cases DESC.
    """
    cache_key = f"ftype_summary:{json.dumps({k:v for k,v in filters.items() if k!='_db'}, sort_keys=True)}"
    cached = _cache_get(cache_key)
    if cached is not None:
        return cached

    f_with_db = _inject_db(filters, db)
    where, params = _build_where(f_with_db)
    e_expr, i_expr, o_expr = _build_metric_exprs(filters)
    total_expr = f"({e_expr} + {i_expr} + {o_expr})"
    ftype_expr = _facility_type_expr()

    sql = text(f"""
        SELECT
            ({ftype_expr})                              AS facility_type,
            COALESCE(SUM({e_expr}), 0)                  AS total_emergency,
            COALESCE(SUM({i_expr}), 0)                  AS total_inpatient,
            COALESCE(SUM({o_expr}), 0)                  AS total_outpatient,
            COALESCE(SUM({total_expr}), 0)              AS total_cases,
