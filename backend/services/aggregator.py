...
def _build_where_facility_only(filters: dict) -> tuple[str, dict]:
    ...
    Strips practitioner_id AND search so the denominator reflects all practitioners
    at the selected region / facility / speciality / date range — matching the
    DAX pattern where only the practitioner-identifying filters are removed.
    """
...
def get_kpi_summary(db: Session, filters: dict) -> dict:
    ...
    # ── Total Visits by Facility(ies) ────────────────────────────────────
    # If a practitioner filter (ID or search) is active, find all facilities 
    # that practitioner works in, then sum ALL visits in those facilities 
    # (removing the practitioner/search filter).
    
    # 1. Filters for the context (region, speciality, date) but NO practitioner
    filters_ctx = {k: v for k, v in filters.items() if k not in ["practitioner_id", "search"]}
    where_ctx, params_ctx = _build_where(filters_ctx)
    
    # 2. Check if we have a practitioner-specific filter (ID or search)
    has_pract_filter = bool(filters.get("practitioner_id") or filters.get("search"))
    
    if has_pract_filter:
        # Find the specific facilities this practitioner/search matches
        # We use the full 'where' to identify their records
        fac_find_sql = text(f"""
            SELECT DISTINCT facility_name
            FROM practitioner_records
            WHERE 1=1 {where}
        """)
        fac_names = [r[0] for r in db.execute(fac_find_sql, params).fetchall()]
        
        if fac_names:
            # Sum cases for these facilities within the context
            p_names = []
            p_vals = {}
            ...
            fac_in_list = ", ".join(p_names)
            denom_sql = text(f"""
                SELECT COALESCE(SUM({total_expr}), 0)
                FROM practitioner_records
                WHERE 1=1 {where_ctx}
                AND facility_name IN ({fac_in_list})
            """)
            fac_total = db.execute(denom_sql, {**params_ctx, **p_vals}).scalar() or 0
        else:
            fac_total = 0
    else:
        # No practitioner filter -> just sum based on existing context (facility/region/etc)
        denom_sql = text(f"""
            SELECT COALESCE(SUM({total_expr}), 0)
            FROM practitioner_records
            WHERE 1=1 {where_ctx}
        """)
        fac_total = db.execute(denom_sql, params_ctx).scalar() or 0

    result["total_visits_by_facility"] = fac_total

    # ── % Practitioner per Facility(ies) ───────────────────────────────
    tc = result.get("total_cases", 0) or 0
    denom = fac_total # The denominator is the total of the facilities involved
    
    # If no filters at all, or if the denominator ended up being the same as tc, 
    # and no practitioner filter is active, it might be 100%. 
    # But for the % card, we always want tc / fac_total.
