...
def _run_migrations():
    ...
    with engine.connect() as conn:
        ...
        existing_pr = [row[1] for row in conn.execute(text("PRAGMA table_info(practitioner_records)")).fetchall()]
        if "month" not in existing_pr:
            conn.execute(text("ALTER TABLE practitioner_records ADD COLUMN month TEXT"))
            conn.commit()

        # ── Add `total_rows` column to imported_files ────────────────────────
        existing_if = [row[1] for row in conn.execute(text("PRAGMA table_info(imported_files)")).fetchall()]
