...
def render_charts(pivot: list[dict], group_by: str, kpi: dict, filters: dict = None):
    ...
    with tabs[0]:
        ...
        # Keep only opts whose column exists in the dataframe
        bar_opts = [(k, lbl) for k, lbl in bar_opts if k in df.columns or k in [
            "total_cases", "total_records", "total_emergency",
            "total_inpatient", "total_outpatient",
        ]]
        bar_keys = [k for k, _ in bar_opts]
        bar_lbls = [lbl for _, lbl in bar_opts]
        bar_sel_lbl = st.selectbox("Metric", bar_lbls, key="bar_metric")
        metric = bar_keys[bar_lbls.index(bar_sel_lbl)]
    ...
    with tabs[2]:
        ...
        pie_opts = [(k, lbl) for k, lbl in pie_opts if k in df.columns or k in [
            "total_cases", "total_records", "total_emergency",
            "total_inpatient", "total_outpatient",
        ]]
        pie_keys = [k for k, _ in pie_opts]
        pie_lbls = [lbl for _, lbl in pie_opts]
        pie_sel_lbl = st.selectbox("Metric", pie_lbls, key="pie_metric")
        pie_metric = pie_keys[pie_lbls.index(pie_sel_lbl)]
