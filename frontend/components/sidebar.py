...
FacilityType filter:
  - Single bilingual multiselect: "PCU - وحدة", "PCC - مركز", etc.
  - Options are a STATIC list (6 fixed categories) — no API call needed.
    Cascading with other filters happens on the summary/data side, not here.
  - The English key is extracted from the label before sending to the backend.
"""
...
# ── FacilityType bilingual mapping ─────────────────────────────────────────
# STATIC list — never requires an API call, so no delay when this filter changes.
# Order: most common first for ergonomics.
_FTYPE_LABELS = [
    ("PCU",      "PCU - وحدة"),
    ("PCC",      "PCC - مركز"),
    ("Hospital", "Hospital - مستشفى"),
    ("Complex",  "Complex - مجمع"),
    ("ICNU",     "ICNU - مركز الكلى والمسالك"),
]
# eng_key → display label
_FTYPE_ENG_TO_LABEL = {eng: lbl for eng, lbl in _FTYPE_LABELS}
# display label → eng_key
_FTYPE_LABEL_TO_ENG = {lbl: eng for eng, lbl in _FTYPE_LABELS}
# all display labels in order
_FTYPE_ALL_LABELS   = [lbl for _, lbl in _FTYPE_LABELS]
...
def render_sidebar() -> dict:
    ...
    # Gather current active filters to drive cascading logic for region/facility/speciality
    curr_cascade: dict = {}
    ...
    # facility_type selection (stored as eng keys in saved_filters)
    ft_eng_saved = saved.get("facility_type", [])

    # We must read the current state of facility_type before get_opts is called
    ft_labels_val = st.session_state.get(f"sb_fac_type_{v}")
    if ft_labels_val is not None:
        ft_eng_val = [_FTYPE_LABEL_TO_ENG[lbl] for lbl in ft_labels_val if lbl in _FTYPE_LABEL_TO_ENG]
    else:
        ft_eng_val = ft_eng_saved

    if ft_eng_val:
        curr_cascade["facility_type"] = ft_eng_val
    ...
    def get_opts(col: str, fallback_list: list[str]) -> list[str]:
        ...
        f_for_col = {k: vv for k, vv in curr_cascade.items() if k != col}
        opts = fallback_list
        if f_for_col:
            cascaded = _get_cascaded(col, json.dumps(f_for_col, sort_keys=True))
            if cascaded:
                opts = cascaded
        return sorted(opts, key=lambda x: str(x).lower())
    ...
    # ── FacilityType (bilingual, single filter) ────────────────────────────
    # Options are STATIC (6 fixed types) — no API call, no delay.
    ft_label_def = [
        _FTYPE_ENG_TO_LABEL[e]
        for e in ft_eng_saved
        if e in _FTYPE_ENG_TO_LABEL
    ]
    facility_type_labels = st.sidebar.multiselect(
        "🏷️ Facility Type / نوع المنشأة",
        options=_FTYPE_ALL_LABELS,
        default=ft_label_def,
        key=f"sb_fac_type_{v}",
    )
    if facility_type_labels:
        # Map display labels back to English keys for the backend
        ft_eng = [_FTYPE_LABEL_TO_ENG[lbl] for lbl in facility_type_labels]
        filters["facility_type"] = ft_eng
    ...
    if tn_enabled:
        ...
        options = [
            ...
            ("total_visits_all_facilities", "🏛️ Total Visits by Facility(ies)"),
