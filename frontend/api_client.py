...
def get_filter_options_cascaded(column: str, active_filters: dict) -> list[str]:
    ...
    for key in ("region", "facility_name", "speciality", "practitioner_id",
                "facility_type", "date_from", "date_to"):
...
def get_facility_type_summary(filters: dict = None) -> list[dict]:
    """Return per-facility-type totals. Fast lightweight query."""
    r = _get("/facility-type-summary", params=filters or {})
    if r.ok:
